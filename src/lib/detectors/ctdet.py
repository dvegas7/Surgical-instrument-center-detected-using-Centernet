...
class CtdetDetector(BaseDetector):
  ...
  def debug(self, debugger, images, dets, output, scale=1):
    ...
    for i in range(1):
      ...
      with open('./array.npy', 'wb') as f:
      	np.save(f,output['hm'][i].detach().cpu().numpy())
  ...
  def center(self,array):
    img_length=512  #default
    img_width=512
    with open(array,'rb') as f:
      array2 = np.load(f)
    local_max = self.find_local_maxima(array2[0]) 
    data = np.column_stack((local_max[1]*(img_length/128), local_max[0]*(img_width/128)))
    np.savetxt('coordinates.txt', data, fmt='%d', delimiter='\t') 
